<?php

namespace App\Models;

use Core\Session;
use Core\Redirect;

use Core\WS_read;
use Core\WS_read_free;
use Core\WS_update;
use Core\WS_write;
use Core\WS_write_free;
use App\Models\Padroes_gerais;




class Testemunho 
{

    
    
    //Busca adm com email para Login
    public static function relatorio_all_testemunho() {
        //Dados obrigatorios
        $array = [
            "funcao" => "relatorio_all_testemunho"
        ];
        $resultado = WS_read::ler_dados($array);
        return  json_decode($resultado);
    }
    
    
    //Busca adm com email para Login
    public static function testemunhos_to_page() {
        //Dados obrigatorios
        $array = [
            "funcao" => "busca_testemunhos_site"
        ];
        $resultado = WS_read_free::read_free($array);
        return  json_decode($resultado);
    }
    
    
    
    
    //Busca adm com email para Login
    public static function cadastrar_novo_testemunho($name, $testimony, $status, $thumb) {
        //Dados obrigatorios
        $array = [
            "funcao"     => "cadastrar_novo_testemunho",
            "name"       => $name,
            "testimony"  => $testimony,
            "status"     => $status,
            "thumb"      => $thumb,
            "created_at" => Padroes_gerais::data_e_hora(),
            "active"     => 1
        ];
        $resultado = WS_write::white($array);
        return  json_decode($resultado);
    }
    
    
    //Busca adm com email para Login
    public static function busca_testemunho_com_id($id) {
        //Dados obrigatorios
        $array = [
            "funcao"  => "busca_testemunho_com_id",
            "id_status" => $id
            
        ];
        $resultado = WS_read::ler_dados($array);
        return  json_decode($resultado);
    }
    
    
    
    
    //Busca adm com email para Login
    public static function altera_testemunho($id_testimony, $active, $name, $testimony, $status, $thumb) {
        //Dados obrigatorios
        $array = [
            "funcao"    => "altera_testemunho",
            "id_testimony"   => $id_testimony,
            "active"         => $active,
            "name"           => $name,
            "testimony"      => $testimony,
            "status"         => $status,
            "thumb"          => $thumb
        ];
        $resultado = WS_update::alterar_dados($array);
        return  json_decode($resultado);
    }
    
    
    
    
    
    
    
    
    
    
   
    
    

    
    

    
    

    
    
    
    
}

    
